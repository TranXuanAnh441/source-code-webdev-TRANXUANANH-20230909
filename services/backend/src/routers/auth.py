...
from src.serializers.userSerializers import userEntity, userResponseEntity
from .. import utils
from src.schemas.user import CreateUserSchema, UserResponse, LoginUserSchema
...
def login(payload: LoginUserSchema, response: Response, Authorize: AuthJWT = Depends()):
    ...
    db_user = User.find_one({'email': payload.email})
    if not db_user:
